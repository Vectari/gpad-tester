...
function assets() {
  if (isProduction) {
    ...
    return gulp
      .src(paths.src.assets, { base: "." })
      .pipe(gulp.dest(paths.dist));
  }

  return gulp
    .src([...paths.src.assets, paths.src.svg], { base: "." })
    .pipe(gulp.dest(paths.dist));
}
...
function watch() {
  ...
  gulp.watch(
    [paths.src.html.main, paths.src.html.templates],
    gulp.series(html, templates)
  );
  gulp.watch(paths.src.lang);
...
const build = gulp.series(
  ...
  bundleAssets,
  gulp.parallel(html, templates, assets)
);
